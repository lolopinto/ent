...
- `prettier` is legacy compatibility for forcing Prettier during codegen. See [PrettierConfig](#prettierconfig) for the expected format.
...
- `custom` (boolean. optional). indicates there's already a [custom prettier configuration](https://prettier.io/docs/en/configuration.html) that should be used. This is only relevant when forcing Prettier during codegen.
- `glob` (string. optional). glob to pass to prettier instead of default `src/ent/**/*.ts`

### Formatting

Generated TypeScript is formatted with Biome during codegen.
...
interface CodegenConfig {
  ...
  // legacy fallback only. prefer biome.json or biome.jsonc at the project root
...
generated state file and by [`tsent prune_schemas`](/docs/advanced-topics/cli#prune_schemas). Runtime
