...
- `prettier` configures legacy Prettier formatting and makes codegen use Prettier instead of Biome. See [PrettierConfig](#prettierconfig) for the expected format.
...
- `custom` (boolean. optional). indicates there's already a [custom prettier configuration](https://prettier.io/docs/en/configuration.html) that should be used.
