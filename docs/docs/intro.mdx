...
* a local [PostgresQL](https://www.postgresql.org/download/) instance installed or [SQLite](https://www.sqlite.org/index.html).
* If using Postgres, a database created via `createdb ent-starter` (or replace with your own database name).
  * if a different database is used, make sure to update the `DB_CONNECTION_STRING`  env in `docker-compose.dev.yml`
* If using SQLite, update the `DB_CONNECTION_STRING` env in `docker-compose.dev.yml` to something like `sqlite:///ent-starter.db` to indicate where the db should be stored.
...
# install all the things in the docker container
npm run init
...
* `src/ent/user.ts` is the public facing API of the `User` object and what's consumed. It's also where [**custom code**](/docs/custom-code/custom-accessors) can be added.
...
* added a [custom accessor](/docs/custom-code/custom-accessors) using [gqlField](/docs/custom-code/gql-field)
