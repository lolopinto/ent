...
```shell
  new file:   src/ent/generated/const.ts
...
Like the change in `create_user_action.ts` above, we've added a `getPrivacyPolicy` method to change [permissions](/docs/core-concepts/privacy-policy) of who can load the ent. The default generated privacy policy for each ent is [`AllowIfViewerPrivacyPolicy`](/docs/core-concepts/privacy-policy#allowifviewerprivacypolicy).
