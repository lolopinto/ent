...
func (p *TSStep) ProcessData(processor *codegen.Processor) error {
	...
	// generate schema.gql
	return generateSchemaFile(processor, p.s)
}
...
func getTempSchemaFilePath(cfg SchemaConfig) string {
...
func generateSchemaFile(processor *codegen.Processor, s *gqlSchema) error {
	var sb strings.Builder
...
// generate schema file based on graphql-js and then compared with
// the one we generate
func GenerateFSBasedSchemaFile(cfg SchemaConfig) error {
	// this generates the schema.gql file
	filePath := getTempSchemaFilePath(cfg)

	err := writeSchemaFile(cfg, filePath)
...
func writeSchemaFile(cfg SchemaConfig, fileToWrite string) error {
	filePath := path.Join(cfg.GetAbsPathToRoot(), "src/graphql/generated/temp_schema.gql")
