...
- release `@snowtop/ent` 0.2.15; use `tsent` v0.3.10 or Docker v0.3.12
  and regenerate builders (#2042, #2043, #2045).
...
- preserve no-op edits when triggers remove all explicit field changes, without
  ...
  still persist. Requires regenerated builders with the updated runtime (#2041).
- give `EntSchemaWithTZ` the same conditional `updatedAt` behavior as
  `EntSchema` and `Timestamps` (#2041).
...
- emit `assoc_edge_config` inserts after table creation in generated `schema.sql` files (#2009).
...
- include Prettier in the development Docker image for legacy codegen formatting (#2000).
