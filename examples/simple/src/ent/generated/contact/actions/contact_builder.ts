  ...
  implements Builder<Contact, ExampleViewerAlias, TExistingEnt>
{
  ...
  addSelfContactForUser(...nodes: (ID | User | Builder<User, any>)[]): this {
    for (const node of nodes) {
      if (this.isBuilder(node)) {
        this.addSelfContactForUserID(node);
      } else if (typeof node === "object") {
        this.addSelfContactForUserID(node.id);
      } else {
        this.addSelfContactForUserID(node);
      }
    }
  ...
  removeSelfContactForUser(...nodes: (ID | User)[]): this {
    for (const node of nodes) {
