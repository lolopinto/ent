...
import CreateNoteAction from "../ent/note/actions/create_note_action.js";
...
import WorkspaceAddMemberAction from "../ent/workspace/actions/workspace_add_member_action.js";
import { semanticChunkSearch } from "./semantic_search.js";
...
describe("semantic notes search integration", () => {
  dbTest(
    "creates semantic notes data and returns workspace-scoped nearest chunks",
    async () => {
      ...
      expect(
        rows.every((row) => row.workspace_id === String(workspace.id)),
      ).toBe(true);
