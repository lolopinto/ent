// Generated by github.com/lolopinto/ent/ent, DO NOT EDIT.

import type { ID, PrivacyPolicy, Viewer } from "@snowtop/ent";
import type {
  Action,
  Changeset,
  ChangesetOptions,
  Observer,
  Trigger,
  Validator,
} from "@snowtop/ent/action";
import type { AccountInput } from "src/ent/generated/account/actions/account_builder";
import { AllowIfViewerHasIdentityPrivacyPolicy } from "@snowtop/ent";
import { WriteOperation } from "@snowtop/ent/action";
import { Account } from "src/ent/";
import { AccountBuilder } from "src/ent/generated/account/actions/account_builder";

export type DeleteAccountActionTriggers = (
  | Trigger<
      Account,
      AccountBuilder<AccountInput, Account>,
      Viewer,
      AccountInput,
      Account
    >
  | Trigger<
      Account,
      AccountBuilder<AccountInput, Account>,
      Viewer,
      AccountInput,
      Account
    >[]
)[];

export type DeleteAccountActionObservers = Observer<
  Account,
  AccountBuilder<AccountInput, Account>,
  Viewer,
  AccountInput,
  Account
>[];

export type DeleteAccountActionValidators = Validator<
  Account,
  AccountBuilder<AccountInput, Account>,
  Viewer,
  AccountInput,
  Account
>[];

export class DeleteAccountActionBase
  implements
    Action<
      Account,
      AccountBuilder<AccountInput, Account>,
      Viewer,
      AccountInput,
      Account
    >
{
  public readonly builder: AccountBuilder<AccountInput, Account>;
  public readonly viewer: Viewer;
  protected readonly account: Account;

  constructor(viewer: Viewer, account: Account) {
    this.viewer = viewer;
    this.builder = new AccountBuilder(
      this.viewer,
      WriteOperation.Delete,
      this,
      account,
    );
    this.account = account;
  }

  getPrivacyPolicy(): PrivacyPolicy<Account, Viewer> {
    return AllowIfViewerHasIdentityPrivacyPolicy;
  }

  getTriggers(): DeleteAccountActionTriggers {
    return [];
  }

  getObservers(): DeleteAccountActionObservers {
    return [];
  }

  getValidators(): DeleteAccountActionValidators {
    return [];
  }

  getInput(): AccountInput {
    return {};
  }

  async changeset(): Promise<Changeset> {
    return this.builder.build();
  }

  async changesetWithOptions_BETA(
    options: ChangesetOptions,
  ): Promise<Changeset> {
    return this.builder.buildWithOptions_BETA(options);
  }

  async valid(): Promise<boolean> {
    return this.builder.valid();
  }

  async validX(): Promise<void> {
    await this.builder.validX();
  }

  async save(): Promise<void> {
    await this.builder.save();
  }

  async saveX(): Promise<void> {
    await this.builder.saveX();
  }

  async hardDelete(): Promise<void> {
    this.builder.orchestrator.setDisableTransformations(true);
    await this.builder.save();
  }

  async hardDeleteX(): Promise<void> {
    this.builder.orchestrator.setDisableTransformations(true);
    await this.builder.saveX();
  }

  static create<T extends DeleteAccountActionBase>(
    this: new (
      viewer: Viewer,
      account: Account,
    ) => T,
    viewer: Viewer,
    account: Account,
  ): T {
    return new this(viewer, account);
  }

  static async saveXFromID<T extends DeleteAccountActionBase>(
    this: new (
      viewer: Viewer,
      account: Account,
    ) => T,
    viewer: Viewer,
    id: ID,
  ): Promise<void> {
    const account = await Account.loadX(viewer, id);
    return new this(viewer, account).saveX();
  }
}
